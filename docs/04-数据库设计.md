...
**注意**：根据最新业务需求，用户表字段约束已调整，真实姓名和手机号现为必填字段，邮箱为可选字段。
...
| email | VARCHAR(100) | UNIQUE | 用户邮箱（可选，如提供需唯一） |
| role | VARCHAR(20) | NOT NULL DEFAULT 'USER' | 用户角色：USER（农户）, ADMIN（管理员） |
| full_name | VARCHAR(100) | NOT NULL | 用户全名（必填） |
| phone_number | VARCHAR(20) | UNIQUE, NOT NULL | 手机号码（必填，需唯一） |
